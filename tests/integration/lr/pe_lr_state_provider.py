...
from tests.integration.lr.pe_lr_flow_state import LrPeFlowTestState


class LrPeTestStateProvider:

    @staticmethod
    def get_state(data_dir):
        input_dir = os.path.join(data_dir, 'integration', 'lr', 'input')
        input_csv_file = os.path.join(input_dir, 'malware_input_full_data.csv')
        output_dir_path = os.path.join(data_dir, 'integration', 'lr', 'output')
        return LrPeTestStateProvider.get_state_by_input_file_and_output_dir(input_csv_file, output_dir_path)
    ...
    def get_state_by_input_file_and_output_dir(input_csv_file, output_dir_path):
        ...
        state.output_model_file = os.path.join(state.output_train_dir_path, 'logistic_regression_model.joblib')

        state.output_evaluate_dir_path = os.path.join(state.output_root_dir_path, 'evaluate')
        state.lr_test_metrics_json_file = os.path.join(state.output_evaluate_dir_path, 'test_metrics.json')
