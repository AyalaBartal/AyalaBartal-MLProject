...
class TestLrPeEvaluatorWriter(unittest.TestCase):

    def setUp(self):
        self.formatter = MagicMock(spec=LrPeEvaluatorFormatter)
        self.file_writer = MagicMock(spec=FileWriter)
        self.plot_writer = MagicMock(spec=MlIoPlotWriter)
        self.writer = LrPeEvaluatorWriter(
            self.formatter,
            self.file_writer,
            self.plot_writer
        )
    ...
    def test_init_sets_dependencies(self):
        formatter = MagicMock(spec=LrPeEvaluatorFormatter)
        file_writer = MagicMock(spec=FileWriter)
        plot_writer = MagicMock(spec=MlIoPlotWriter)

        writer = LrPeEvaluatorWriter(formatter, file_writer, plot_writer)
