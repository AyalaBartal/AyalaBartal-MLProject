...
class TestLrPeLogicTrainer(unittest.TestCase):

    def setUp(self):
        self.reader = create_autospec(LrPeDataTrainer, instance=True)
        self.trainer = create_autospec(LrPeModelTrainer, instance=True)
        self.reporter = create_autospec(LrPeReportTrainer, instance=True)

        self.logic_trainer = LrPeLogicTrainer(
            reader=self.reader,
    ...
    def test_train_runs_full_pipeline_and_returns_report(self):
        args = Mock(spec=LrPeTrainAlgoArgs)
        ...
        self.trainer.get_logistic_regression_classifier.return_value = model_before_fit
        ...
        self.trainer.get_logistic_regression_classifier.assert_called_once_with(args)
    ...
    def test_train_calls_methods_in_expected_order(self):
        ...
        self.trainer.get_logistic_regression_classifier.side_effect = log_and_return("get_logistic_regression_classifier", model_before_fit)
    ...
    def test_init_raises_when_reader_has_wrong_type(self):
        with self.assertRaises(TypeError) as context:
            LrPeLogicTrainer(object(), self.trainer, self.reporter)

        self.assertEqual(
            "reader must be of type LrPeDataTrainer, but got object",
            str(context.exception),
        )

    def test_init_raises_when_trainer_has_wrong_type(self):
        with self.assertRaises(TypeError) as context:
            LrPeLogicTrainer(self.reader, object(), self.reporter)

        self.assertEqual(
            "trainer must be of type LrPeModelTrainer, but got object",
            str(context.exception),
        )

    def test_init_raises_when_reporter_has_wrong_type(self):
        with self.assertRaises(TypeError) as context:
            LrPeLogicTrainer(self.reader, self.trainer, object())

        self.assertEqual(
            "reporter must be of type LrPeReportTrainer, but got object",
            str(context.exception),
        )
