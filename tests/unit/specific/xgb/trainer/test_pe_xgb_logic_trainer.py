...
class TestXgbPeLogicTrainer(unittest.TestCase):

    def setUp(self):
        self.reader = create_autospec(XgbPeDataTrainer, instance=True)
        self.trainer = create_autospec(XgbPeModelTrainer, instance=True)
        self.reporter = create_autospec(XgbPeReportTrainer, instance=True)

        self.logic_trainer = XgbPeLogicTrainer(
            reader=self.reader,
    ...
    def test_train_runs_full_pipeline_and_returns_report(self, mock_provider):
        ...
        self.trainer.get_xgboost_classifier.return_value = model_before_fit
        ...
        self.trainer.get_xgboost_classifier.assert_called_once_with(args)
    ...
    def test_train_processes_data_in_correct_order(self, mock_provider):
        ...
        # Verify label extracted before features
    ...
    def test_init_raises_when_reader_has_wrong_type(self):
        with self.assertRaises(TypeError) as context:
            XgbPeLogicTrainer(object(), self.trainer, self.reporter)

        self.assertEqual(
            "reader must be of type XgbPeDataTrainer, but got object",
            str(context.exception),
        )

    def test_init_raises_when_trainer_has_wrong_type(self):
        with self.assertRaises(TypeError) as context:
            XgbPeLogicTrainer(self.reader, object(), self.reporter)

        self.assertEqual(
            "trainer must be of type XgbPeModelTrainer, but got object",
            str(context.exception),
        )

    def test_init_raises_when_reporter_has_wrong_type(self):
        with self.assertRaises(TypeError) as context:
            XgbPeLogicTrainer(self.reader, self.trainer, object())

        self.assertEqual(
            "reporter must be of type XgbPeReportTrainer, but got object",
            str(context.exception),
        )
