...
class TestLgbPeEvaluatorReader(unittest.TestCase):

    def setUp(self):
        self.validator = MagicMock()
        self.reader = LgbPeEvaluatorReader(self.validator)
    ...
    def test_validate_input_calls_validator(self):
        args_input = LgbPeEvaluateInputArgs("input_dir")
    ...
    @patch('src.specific.lgb.evaluate.pe_lgb_evaluator_reader.pd')
    ...
    @patch('src.specific.lgb.evaluate.pe_lgb_evaluator_reader.load')
    def test_read_lgb_model_from_joblib_file_loads_model(self, mock_load):
        mock_model = MagicMock()
        mock_load.return_value = mock_model

        args_input = LgbPeEvaluateInputArgs("path/to/input")
        actual = self.reader.read_lgb_model_from_joblib_file(args_input)

        self.assertIs(actual, mock_model)
        mock_load.assert_called_once_with(args_input.input_model)

    @patch('src.specific.lgb.evaluate.pe_lgb_evaluator_reader.load')
    def test_read_lgb_model_from_joblib_file_with_different_path(self, mock_load):
        mock_model = MagicMock()
        mock_load.return_value = mock_model
