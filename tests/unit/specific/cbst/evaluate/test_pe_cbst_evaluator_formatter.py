...
class TestCbstPeEvaluatorFormatter(unittest.TestCase):
    ...
    def test_get_json_from_report_threshold_with_different_values(self):
        report = CbstPeEvaluateReport(auc=0.75, acc=0.80, cm=np.array([[5, 3], [2, 10]]))
        threshold = 0.6
    ...
    def test_get_md_from_report_threshold_with_high_scores(self):
        report = CbstPeEvaluateReport(auc=0.99, acc=0.97, cm=np.array([[100, 1], [2, 97]]))
        threshold = 0.5
    ...
    def test_get_md_from_report_threshold_with_low_scores(self):
        report = CbstPeEvaluateReport(auc=0.51, acc=0.52, cm=np.array([[26, 24], [24, 26]]))
        threshold = 0.5
